...
const Dashboard: React.FC = () => {
  ...
  const hardiness_zone = useSelector<AppStore, string>(state => state.userInfoReducer.hardiness_zone);

  const growing_season_length = useSelector<AppStore, number>(state => state.userInfoReducer.growing_season_length);

  const first_gdd35 = useSelector<AppStore, string>(state => state.userInfoReducer.first_gdd35);

  const last_gdd35 = useSelector<AppStore, string>(state => state.userInfoReducer.last_gdd35);


  const userMethods = useSelector<AppStore, ManagementMethod[]>(state => state.mgmtMethodReducer.userMethods);
