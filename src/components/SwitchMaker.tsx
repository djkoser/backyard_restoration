...
const SwitchMaker: React.FC<SwitchMakerProps> = (props) => {
  ...
  let checked = false;
  if (userMethods.reduce((acc: number, el) => weedMethod.method_id === el.method_id ? ++acc : acc, 0)) {
    checked = true;
  }
  return (
    <div className='switchBody'>
      <div className='switchPara'>
        ...
        <div className='switchFlexBox'>
          <div style={checked ? undefined : { transform: 'translateY(15px)' } as CSS.Properties} className="switch" >
            <h5 style={checked ? undefined : { transform: 'translateY(-6px)' } as CSS.Properties} className="switchLabel">{checked ? 'Remove from Timeline' : 'Add to Timeline'}</h5>
            <input type="checkbox"
