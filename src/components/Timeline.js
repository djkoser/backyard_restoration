...
const Timeline = (props) => {
  ...
  // @ts-ignore
  const user_id = useSelector(state => state.userInfoReducer.user_id)
  ...
  const prevUser = usePreviousValue(user_id)
  ...
  useEffect(
    () => {
      if (userMethods && first_gdd35 && last_gdd35 && user_id) {
  ...
  return (
    <main className="d3Container">
      ...
      <svg
        ref={removeReference ? undefined : d3Container}
