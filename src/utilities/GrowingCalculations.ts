...
export class GrowingCalculations {
  ...
  private minimumSeasonStartQualifierPercentage = 0.9;
  // The minimum percentage of leading days with non-zero (season start) or zero (season end) growing degree days in order to qualify it as a season start/end.
    ...
    if (this.tminMaxMap)
      this.tminMaxMap.forEach((value, key) => {
        ...
        if (
          ...
          GDD !== undefined &&
          GDD > 0 &&
          lagNonZeroGDDP <= this.maximumLagQualifierPercentage &&
          leadNonZeroGDDP >= this.minimumSeasonStartQualifierPercentage
        ) {
          gdd45SpringTransitionDates.push(new Date(key));
        }
      });
    gdd45SpringTransitionDates.forEach((date) =>
    ...
    if (this.tminMaxMap)
      this.tminMaxMap.forEach((value, key) => {
        ...
        if (
          ...
          GDD !== undefined &&
          GDD > 0 &&
          lagZeroGDDP <= this.maximumLagQualifierPercentage &&
          leadZeroGDDP >= this.minimumSeasonEndQualifierPercentage
        ) {
          gdd45WinterTransitions.push(new Date(key));
        }
      });
    gdd45WinterTransitions.forEach((date) =>
      date.setFullYear(this.currentDate.getFullYear())
    );
   ...
   */
  private getLeadAndLagGDDs(observationDate: string): {
    leadGDDs: number[];
    lagGDDs: number[];
  } {
    ...
    return {
      leadGDDs: leadingObservationDates
        .map((obsDate) => {
          const mapValue = this.tminMaxMap
            ? this.tminMaxMap.get(obsDate)
            : undefined;
  ...
  private mapTMINTMAX(TMIN: Observation[], TMAX: Observation[]): void {
    ...
    TMIN.forEach(({ temperature, observation_date }) => {
      const observationDateParsed =
        observation_date.match(/\d\d\d\d-\d\d-\d\d/)?.[0] || '';
      if (this.tminMaxMap)
        this.tminMaxMap.set(observationDateParsed, { TMIN: temperature });
    });
    TMAX.forEach(({ temperature, observation_date }) => {
      const observationDateParsed =
        observation_date.match(/\d\d\d\d-\d\d-\d\d/)?.[0] || '';
      if (this.tminMaxMap && this.tminMaxMap.has(observationDateParsed)) {
        const existing = this.tminMaxMap.get(observationDateParsed) as {
          TMIN: number;
          TMAX?: number;
          GDD?: number;
        };
    ...
    if (this.tminMaxMap)
      this.tminMaxMap.forEach((value, key) => {
        ...
        value.lagNonZeroGDDP = lagGDDs.length
          ? lagGDDs.reduce((count, gdd) => (gdd > 0 ? count + 1 : count), 0) /
            lagGDDs.length
          : undefined;
        ...
        value.leadNonZeroGDDP = leadGDDs.length
          ? leadGDDs.reduce((count, gdd) => (gdd > 0 ? count + 1 : count), 0) /
            leadGDDs.length
          : undefined;
  ...
  private async retrieveAPIData(): Promise<WeatherAPIReturn> {
    ...
    let apiOutputs = await this.returnResultsFromFirstStationWithData(
      coordinateString
    );
    while (!apiOutputs) {
      ...
      apiOutputs = await this.returnResultsFromFirstStationWithData(
        coordinateString
      );
      await this.delay(300);
    }
    return apiOutputs.reduce(
      (apiOutputAcc, currentOutput) => {
        return {
          TMIN: [...apiOutputAcc.TMIN, ...currentOutput.TMIN],
          TMAX: [...apiOutputAcc.TMAX, ...currentOutput.TMAX]
        };
      },
      { TMIN: [], TMAX: [] }
    ) as WeatherAPIReturn;
  ...
  private async getTMINandTMAXFromNOAA(
    ...
    try {
      ...
      console.warn(
        `Unable to get temperature data for start date: ${startDate} and end date: ${endDate} due to the following error: ${
   ...
   */
  private async returnResultsFromFirstStationWithData(
    ...
    if (stationList.results) {
      ...
      for (
        let stationIndex = 0;
        stationIndex < stationList.results.length;
        stationIndex++
      ) {
        if (!stationList.results?.[stationIndex]?.id) {
          console.warn(
            `returnResultsFromFirstStationWithData: station at index ${stationIndex} does not contain an ID, continuing to the next ->`
          );
          continue;
        }
        const tempData = await this.getTMINandTMAXFromNOAA(
          edString,
          sdString,
        ...
        if (tempData.TMAX.length && tempData.TMIN.length) {
          ...
          for (
            let yearIndex = 0;
            yearIndex < this.numberOfYearsToRetrieve;
            yearIndex++
          ) {
            promises.push(
  ...
  private selectHighestDuplicates(
    ...
    return Object.entries(observationDateMap).map(
      ([observation_date, temperature]) => {
        return { observation_date, temperature };
      }
    );
  }
  ...
  private date2String(dateParam = new Date()): string {
    return `${dateParam.getFullYear()}-${(
      '0' +
      (dateParam.getMonth() + 1)
    ).slice(-2)}-${('0' + dateParam.getDate()).slice(-2)}`;
  }
  ...
  private averageSeasonLength(
    averageSeasonEndString: string,
    averageSeasonStartString: string
  ) {
    return (
