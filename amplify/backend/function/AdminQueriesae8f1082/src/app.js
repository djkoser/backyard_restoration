...
var cognitoActions_1 = require("./cognitoActions");
...
var checkGroup = function (req, _res, next) {
    ...
    if (req.apiGateway.event.requestContext.authorizer.claims['cognito:groups']) {
        var groups = req.apiGateway.event.requestContext.authorizer.claims['cognito:groups'].split(',');
        if (!(allowedGroup && groups.indexOf(allowedGroup) > -1)) {
            var err = new Error("User does not have permissions to perform administrative tasks");
            next(err);
        }
    }
    else {
        var err = __assign(__assign({}, new Error("User does not have permissions to perform administrative tasks")), { statusCode: 403 });
        next(err);
    }
    next();
};
app.all('*', checkGroup);
app["delete"]('/deleteUserIfUnconfirmed', function (req, res, next) { return __awaiter(void 0, void 0, void 0, function () {
    var err, response, err_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            ...
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, (0, cognitoActions_1.deleteUserIfUnconfirmed)(req.body.username)];
            case 2:
                response = _a.sent();
