...
var GrowingCalculations = /** @class */ (function () {
      ...
      return __generator(this, function (_b) {
        switch (_b.label) {
          ...
          case 1:
            ...
            seasonStarts = this.removeOutliersFromDates(
              this.gdd45SpringTransitions()
            );
            seasonEnds = this.removeOutliersFromDates(
  ...
  GrowingCalculations.prototype.removeOutliersFromDates = function (dates) {
    var values = dates.map(function (dateString) {
  ...
  GrowingCalculations.prototype.gdd45SpringTransitions = function () {
    ...
    if (this.tminMaxMap)
      this.tminMaxMap.forEach(function (value, key) {
        var lagNonZeroGDDP = value.lagNonZeroGDDP,
          leadNonZeroGDDP = value.leadNonZeroGDDP,
          GDD = value.GDD;
  ...
  GrowingCalculations.prototype.gdd45WinterTransitions = function () {
    ...
    if (this.tminMaxMap)
      this.tminMaxMap.forEach(function (value, key) {
        var lagZeroGDDP = value.lagZeroGDDP,
          leadZeroGDDP = value.leadZeroGDDP,
          GDD = value.GDD;
  ...
  GrowingCalculations.prototype.getLowestTemperature = function (TMIN) {
    return TMIN.reduce(function (lowestTemp, tminObs) {
      return tminObs.temperature < lowestTemp
        ? tminObs.temperature
        : lowestTemp;
    }, 9999);
  };
  ...
  GrowingCalculations.prototype.mapTMINTMAX = function (TMIN, TMAX) {
    ...
    TMIN.forEach(function (_a) {
      ...
      var temperature = _a.temperature,
        observation_date = _a.observation_date;
      var observationDateParsed =
        ((_b = observation_date.match(/\d\d\d\d-\d\d-\d\d/)) === null ||
        _b === void 0
          ? void 0
          : _b[0]) || '';
      if (_this.tminMaxMap)
    ...
    if (this.tminMaxMap)
      this.tminMaxMap.forEach(function (value, key) {
        var _a = _this.getLeadAndLagGDDs(key),
          lagGDDs = _a.lagGDDs,
          leadGDDs = _a.leadGDDs;
        value.lagNonZeroGDDP = lagGDDs.length
          ? lagGDDs.reduce(function (count, gdd) {
              return gdd > 0 ? count + 1 : count;
            }, 0) / lagGDDs.length
          : undefined;
        ...
        value.leadNonZeroGDDP = leadGDDs.length
          ? leadGDDs.reduce(function (count, gdd) {
              return gdd > 0 ? count + 1 : count;
            }, 0) / leadGDDs.length
          : undefined;
  ...
  GrowingCalculations.prototype.retrieveAPIData = function () {
    ...
    return __awaiter(this, void 0, void 0, function () {
      ...
      return __generator(this, function (_d) {
        switch (_d.label) {
          case 0:
            return [4 /*yield*/, this.convertAddressToCoordinate()];
          case 1:
            googleResult = _d.sent();
            googleResults =
              (_c =
                (_b =
                  (_a = googleResult.results) === null || _a === void 0
                    ? void 0
                    : _a[0]) === null || _b === void 0
                  ? void 0
            ...
            if (googleResults === undefined)
              throw new Error(
                "retrieveAPIData: lat and lng could not be obtained from the user's address via Google Maps"
              );
            ...
            return [
              4 /*yield*/,
              this.returnResultsFromFirstStationWithData(coordinateString)
            ];
          case 2:
            apiOutputs = _d.sent();
            _d.label = 3;
          ...
          case 6:
            return [
              2 /*return*/,
              apiOutputs.reduce(
                function (apiOutputAcc, currentOutput) {
                  return {
                    TMIN: __spreadArray(
                      __spreadArray([], apiOutputAcc.TMIN, true),
                      currentOutput.TMIN,
                      true
                    ),
                    TMAX: __spreadArray(
                      __spreadArray([], apiOutputAcc.TMAX, true),
                      currentOutput.TMAX,
                      true
                    )
                  };
   ...
   */
  GrowingCalculations.prototype.getTMINandTMAXFromNOAA = function (
    endDate,
    startDate,
    stationID
  ) {
      ...
      return __generator(this, function (_b) {
        switch (_b.label) {
          case 0:
            ...
            return [
              ...
              (0, node_fetch_1['default'])(
                'https://www.ncdc.noaa.gov/cdo-web/api/v2/data?datasetid=GHCND&datatypeid=TMIN&datatypeid=TMAX&units=standard&limit=1000&startdate='
                  .concat(startDate, '&enddate=')
                  .concat(endDate, '&stationid=')
                  .concat(stationID, '&includemetadata=false'),
                { method: 'get', headers: { token: this.NOAA_TOKEN } }
              )
            ];
          case 1:
            response = _b.sent();
            ...
            if (
              ...
              data.results.forEach(function (el) {
                if (el.datatype.toLowerCase() === 'tmin') {
                  TMIN_1.push({
                    temperature: el.value,
                    observation_date: el.date
                  });
          ...
          case 5:
            err_1 = _b.sent();
            console.warn(
              'Unable to get temperature data for start date: '
                .concat(startDate, ' and end date: ')
                .concat(endDate, ' due to the following error: ')
                .concat(
   ...
   */
  GrowingCalculations.prototype.returnResultsFromFirstStationWithData =
        ...
        return __generator(this, function (_c) {
          switch (_c.label) {
            case 0:
              ...
              sd = this.less1Year(
                new Date(this.currentDate.getFullYear() - 1, 12, 0)
              );
            ...
            case 1:
              ...
              _c.label = 2;
            case 2:
              if (!(stationIndex < stationList.results.length))
                return [3 /*break*/, 10];
              if (
                !((_b =
                  (_a = stationList.results) === null || _a === void 0
                    ? void 0
                    : _a[stationIndex]) === null || _b === void 0
                  ? void 0
                  : _b.id)
              ) {
                console.warn(
                  'returnResultsFromFirstStationWithData: station at index '.concat(
                    stationIndex,
                    ' does not contain an ID, continuing to the next ->'
                  )
                );
                return [3 /*break*/, 9];
              }
              return [
                4 /*yield*/,
                this.getTMINandTMAXFromNOAA(
                  edString,
                  sdString,
                  stationList.results[stationIndex].id
                )
              ];
            case 3:
              tempData = _c.sent();
            ...
            case 4:
              ...
              _c.label = 5;
            case 5:
              if (!(yearIndex < this.numberOfYearsToRetrieve))
                return [3 /*break*/, 8];
              promises.push(
                this.getTMINandTMAXFromNOAA(
                  edString,
  ...
  GrowingCalculations.prototype.selectHighestDuplicates = function (TMAX) {
    ...
    TMAX.forEach(function (obsTmax) {
      var observation_date = obsTmax.observation_date,
        temperature = obsTmax.temperature;
  ...
  GrowingCalculations.prototype.selectLowestDuplicates = function (TMIN) {
    ...
    TMIN.forEach(function (obsTmin) {
      var observation_date = obsTmin.observation_date,
        temperature = obsTmin.temperature;
      if (observationDateMap[observation_date] !== undefined) {
        var existing = observationDateMap[observation_date];
        if (temperature < existing) {
          observationDateMap[observation_date] = temperature;
        ...
        switch (_a.label) {
          case 0:
            return [
              4 /*yield*/,
              (0, node_fetch_1['default'])(
                'https://www.ncdc.noaa.gov/cdo-web/api/v2/stations?extent='.concat(
                  boundingBox,
                  '&dataset=GHCND&datatypeid=TMIN&datatypeid=TMAX&limit=1000'
                ),
                { headers: { token: this.NOAA_TOKEN }, method: 'get' }
              )
            ];
          case 1:
            response = _a.sent();
            ...
            return [
              ...
              (0, node_fetch_1['default'])(
                ...
                { headers: { token: this.NOAA_TOKEN_BACKUP }, method: 'get' }
              )
            ];
          case 2:
            response = _a.sent();
   ...
   */
  GrowingCalculations.prototype.hardinessZoneCalculator = function (
    averageTMIN
  ) {
    var zoneMap = [
   ...
   */
  GrowingCalculations.prototype.boundingBox = function (
    latitudeInDegrees,
    longitudeInDegrees,
    halfSideInKm
  ) {
  ...
  GrowingCalculations.prototype.date2String = function (dateParam) {
    ...
    return ''
      .concat(dateParam.getFullYear(), '-')
      .concat(('0' + (dateParam.getMonth() + 1)).slice(-2), '-')
      .concat(('0' + dateParam.getDate()).slice(-2));
  };
  ...
  GrowingCalculations.prototype.avgDateString = function (seasonStartsEnds) {
    if (seasonStartsEnds.length) {
      ...
      var datesAsMs = seasonStartsEnds.map(function (el) {
        return el.getTime();
      });
      // Sum and divide by normalized Dates' length creating a new date
      var averageDate = new Date(
        datesAsMs.reduce(function (prev, next) {
          return prev + next;
        }) / seasonStartsEnds.length
      );
  ...
  GrowingCalculations.prototype.averageSeasonLength = function (
    averageSeasonEndString,
    averageSeasonStartString
  ) {
    return (
      (new Date(averageSeasonEndString).getTime() -
