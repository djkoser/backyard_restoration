  ...
  getGrowingParams: async (zipcode, street, city, state, db) => {
    try {
      ...
      const getDataFromNOAA = async (edString, sdString, stationList) => {
        ...
        await axios
          .get(
            `https://www.ncdc.noaa.gov/cdo-web/api/v2/data?datasetid=GHCND&datatypeid=TMAX&datatypeid=TMIN&units=standard&startdate=${sdString}&enddate=${edString}&limit=1000&${stationList}includemetadata=false`,
            { headers: { token: NOAA_TOKEN } }
          )
          .then((res) => {
            res.data.results.forEach((el) =>
              el.datatype === 'TMIN'
                ? TMIN.push({ value: el.value, date: el.date })
                : TMAX.push({ value: el.value, date: el.date })
            );
          })
          .catch((err) => {
            console.log(err);
      ...
      const apiLogic = async (coordinateString) => {
        ...
        await axios
          .get(
            `https://www.ncdc.noaa.gov/cdo-web/api/v2/stations?extent=${coordinateString}&dataset=GHCND&datatypeid=TMIN&datatypeid=TMAX&endDate=${edString}&limit=1000`,
            { headers: { token: NOAA_TOKEN } }
          )
          .then(async (res) => {
            ...
            if (res.data.results) {
              ...
              while (!foundData && index < res.data.results.length) {
                ...
                await axios
                  .get(
                    `https://www.ncdc.noaa.gov/cdo-web/api/v2/data?datasetid=GHCND&datatypeid=TMAX&datatypeid=TMIN&units=standard&startdate=${sdString}&enddate=${edString}&limit=1000&${stationString}includemetadata=false`,
                    { headers: { token: NOAA_TOKEN } }
                  )
                  .then(async (res) => {
      ...
      // Data processing steps -> after receipt of API data
      ...
      return await calculateGParams(db, thisTMINMAX.TMAX, thisTMINMAX.TMIN);
    } catch (err) {
      console.log(err);
    }
  }
